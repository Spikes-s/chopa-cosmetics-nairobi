...
const VariantManager = ({ variantGroups, onChange }: VariantManagerProps) => {
  ...
  const addOption = (groupIndex: number) => {
    ...
    updated[groupIndex] = {
      ...group,
      options: [...group.options, { name: trimmed }],
    };
    onChange(updated);
    setNewOptionValue('');
  };
        ...
        <div key={group.type} className="space-y-2 p-3 rounded-lg border border-border bg-background">
          ...
          <div className="flex flex-wrap gap-2">
            {group.options.map((opt, oi) => (
              <Badge key={opt.name} variant="secondary" className="flex items-center gap-1.5 pr-1">
                {opt.name}
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder={`Add ${group.label.toLowerCase()} option (e.g. ${group.type === 'weight' ? '250g, 500g' : group.type === 'capacity' ? '250ml, 500ml' : group.type === 'size' ? 'Small, Medium' : 'Pack of 3'})`}
